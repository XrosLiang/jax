...
class Array:
  ...
  def __repr__(self):
    dtypestr = (self._eltTy._dtype.name if isinstance(self._eltTy, BaseType)
                else f'BInt{{≤{self._eltTy._bound}}}')
...
def _dynamic_xla_call_transpose(cts_in, *args, jaxpr, num_consts):
  ...
  consts_bar, args_bar = backward_pass(jaxpr, dim_args, args, cts_in)
...
def backward_pass(jaxpr, dim_args, args, cts_in):
  ...
  def read_primal(v):
    if type(v) is core.Literal:
      breakpoint()  # TODO handle literals
    else:
...
def _dynamic_xla_call_vmap(args, in_dims, *, jaxpr, num_consts):
  ...
  assert all(d is batching.not_batched for d in in_dim_bdims)
...
# TODO make core.mapped_aval extensible, make BatchTracer.aval use it
def _batch_tracer_aval(self):
  aval = core.raise_to_shaped(core.get_aval(self.val))
  if self.batch_dim is batching.not_mapped:
    return aval
  else:
    return mapped_aval(aval.shape[self.batch_dim], self.batch_dim, aval)
batching.BatchTracer.aval = property(_batch_tracer_aval)  # type: ignore

def mapped_aval(size:int, axis:int, aval:core.AbstractValue):
  if isinstance(aval, AbsArray):
    return AbsArray(tuple_delete(aval.shape, axis), aval._eltTy)
  else:
    return core.mapped_aval(size, axis, aval)
